...
dk1 = np.array(data1['nof_kernels'])*32*1e-9 
dk2 = np.array(data2['nof_kernels'])*32*1e-9
dk3 = np.array(data3['nof_kernels'])*32*1e-9
dk4 = np.array(data4['nof_kernels'])*32*1e-9
dk5 = np.array(data5['nof_kernels'])*32*1e-9
dk6 = np.array(data6['nof_kernels'])*32*1e-9
dk7 = np.array(data7['nof_kernels'])*32*1e-9
dk8 = np.array(data8['nof_kernels'])*32*1e-9
dk9 = np.array(data9['nof_kernels'])*32*1e-9
...
max8 = scale/np.array(data8['max'])/4*dk1
...
plt.title('Random walk with multiple kernels')
plt.xlabel('Number of parallel Kernels')
plt.xticks(range(0,17))
plt.xlim([0.5,16.5])
plt.ylabel('Throughput GB/s')
fig.savefig('rand-walk-mult-kernel.pdf', format='pdf',bbox_inches='tight')
plt.show()
