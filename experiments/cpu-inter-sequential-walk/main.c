...
static inter_t interferenceMethod = 0;
...
static int initThreads(param_t *params) {
	...
	int s = pthread_barrier_init(&barrier, NULL, MAX_CORES);
	if (s != 0)
...
static void PrintUsage(const char *name) {
    printf("Usage: %s <# of intervals> <size in KB> "
            "<interference method (0rnd, 1seq or 2no) "
            "<use zerocopy (0/1)> "
...
int main(int argc, char **argv) {
    ...
    interferenceMethod = atoi(argv[3]);
    if (interferenceMethod < INTER_RAND || interferenceMethod > INTER_NO) {
        printf("Interference method must be 0,1 or 2. Got %s\n", argv[3]);
        return EXIT_FAILURE;
    }
    
    int useZeroCopy = atoi(argv[4]);
    if (useZeroCopy < 0 || useZeroCopy > 1) {
        printf("Use zerocopy must be 0 or 1. Got %s\n", argv[4]);
        return EXIT_FAILURE;
    }
    ...
    params.useZeroCopy = useZeroCopy;
